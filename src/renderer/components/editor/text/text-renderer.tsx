...
export function exportText({
  ...
  const escapedText = ann.text
    ...
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;');
  result += `<text x="${textX}" y="${textY}" fill="${ann.fill}" font-size="${fontSize}" font-family="${fontFamily}" font-weight="${TEXT_FONT_WEIGHT}" dominant-baseline="text-before-edge">${escapedText}</text>`;
  result += '</g>';
