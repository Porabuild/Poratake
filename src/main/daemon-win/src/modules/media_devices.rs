use super::camera_devices::enumerate_cameras as enumerate_camera_devices;
...
pub fn enumerate_cameras() -> std::result::Result<Vec<MediaDevice>, String> {
    let _apartment = ComApartment::initialize().map_err(|error| error.to_string())?;
    let _media_foundation = MediaFoundation::initialize().map_err(|error| error.to_string())?;
    enumerate_camera_devices().map(|devices| {
        devices
            .into_iter()
...
fn allocated_string(value: PWSTR) -> Result<String> {
    if value.is_null() {
